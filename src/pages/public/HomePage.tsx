...
const roleCards = [
  {
    ...
    description: 'Manage branches, role-based access, exam activity, and reporting from one coordinated platform.',
  },
  {
    title: 'For teachers',
    description: 'Build question banks, create exams, review analytics, and track student performance without juggling tools.',
  },
  {
    title: 'For students',
    description: 'Attempt live exams, review results, follow progress, and stay updated with notifications in one portal.',
  },
  {
    title: 'For parents',
    description: 'Monitor linked children, exam schedules, results, and learning trends without entering the student workspace.',
  },
];
...
const featureCards = [
  ...
  'Result, leaderboard, and notification experiences that keep everyone aligned in real time.',
...
export function HomePage() {
  ...
  return (
    <PublicPageShell
      ...
      description="AI School brings together organization access, question banks, exam delivery, analytics, student performance, and parent visibility in a single platform built for modern academic teams."
    >
      <div className="space-y-6 sm:space-y-8">
        ...
        >
          <div>
            <p className={`text-xs font-semibold uppercase tracking-[0.28em] ${isDark ? 'text-emerald-300' : 'text-emerald-600'}`}>Product landing</p>
            <h2 className={`mt-4 max-w-2xl text-3xl font-semibold leading-tight sm:text-4xl lg:text-[3.4rem] ${isDark ? 'text-slate-50' : 'text-slate-950'}`}>
              Manage teaching, learning, and exam workflows with one connected system.
            </h2>
            <p className={`mt-5 max-w-2xl text-sm leading-7 sm:text-base ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
              From organization onboarding and secure login to student results and parent visibility, the platform keeps academic operations streamlined, readable, and role-aware from the first click.
            </p>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
            <div className={`rounded-[28px] border p-5 ${isDark ? 'border-slate-800 bg-slate-900/90' : 'border-slate-200 bg-slate-50/90'}`}>
              <p className={`text-xs font-semibold uppercase tracking-[0.22em] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>What the app manages</p>
              <p className={`mt-3 text-lg font-semibold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>Organizations, exams, analytics, results, and communication.</p>
            </div>
            <div className={`rounded-[28px] border p-5 ${isDark ? 'border-emerald-900/60 bg-emerald-950/40' : 'border-emerald-200 bg-emerald-50'}`}>
              <p className={`text-xs font-semibold uppercase tracking-[0.22em] ${isDark ? 'text-emerald-300' : 'text-emerald-700'}`}>Built for clarity</p>
              <p className={`mt-3 text-sm leading-7 ${isDark ? 'text-emerald-100/90' : 'text-emerald-900'}`}>Every role sees the right workspace, with focused actions and cleaner academic workflows.</p>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <div className={`rounded-[28px] border p-6 ${isDark ? 'border-slate-800 bg-slate-950/75' : 'border-white/70 bg-white/85'}`}>
            ...
            <h3 className={`mt-3 text-2xl font-semibold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>A cleaner way to coordinate academic work.</h3>
            ...
            <div className={`mt-8 rounded-[28px] border p-5 ${isDark ? 'border-blue-900/60 bg-blue-950/30' : 'border-blue-200 bg-blue-50/80'}`}>
              ...
              <p className={`mt-2 text-sm leading-7 ${isDark ? 'text-blue-100/90' : 'text-blue-900/80'}`}>
                Start with organization login if your team already uses AI School, or register your organization to begin the onboarding conversation.
              </p>
            </div>
          </div>
        </section>
